...
<div>
  ...
  <?php
	if (empty($listRow['onDate']))
	{
		//echo date("d M Y");
		?>
    <input type="text" id="onDate" name="onDate" maxlength="12" size="12" readonly="readonly" value="<?php echo date("Y-m-d"); ?>" onclick="javascript:NewCssCal('onDate','yyyymmdd','arrow','','','','')" /> <img src="../datepicker/cal.gif" alt="" onclick="javascript:NewCssCal('onDate','yyyymmdd','arrow','','','','')" style="cursor:pointer;" />
    <?php		
	}
	else
	{
		?>
    <input type="text" id="onDate" name="onDate" maxlength="12" size="12" readonly="readonly" value="<?php echo $listRow['onDate']; ?>" onclick="javascript:NewCssCal('onDate','yyyymmdd','arrow','','','','')" /> <img src="../datepicker/cal.gif" alt="" onclick="javascript:NewCssCal('onDate','yyyymmdd','arrow','','','','	')" style="cursor:pointer;" />
    <?php
	...
	<span style="padding-right:34px;">Page Title : </span> <span>
  <input type="text" name="listPageTitle" value="<?php echo $listRow['pageTitle']; ?>" class="text" />
  </span> <br class="clearboth" />
	<span style="padding-right:15px;">Page Keyword : </span> <span>
  <input type="text" name="listPageKeyword" value="<?php echo $listRow['pageKeyword']; ?>" class="text" />
  </span> <br class="clearboth" />
  <span style="padding-right:1px;">Page Description : </span> <span>
  <input type="text" name="listPageDescription" value="<?php echo $listRow['pageDescription']; ?>" class="text" style="width:400px;" />
  </span> <br class="clearboth" />
  <span style="padding-right:61px;">Title : </span> <span>
  <input type="text" name="listTitle" id="listTitle" value="<?php echo $listRow['name']; ?>" class="text" onchange="copySame('listUrlTitle', this.value);" /></span>
	<br class="clearboth" />
	<span style="padding-right:35px;">Alias Title : </span> <span>
  ...
  <?php
	...
	$oFCKeditor->Create();*/
	?>
	<textarea name="listDescription" class="ckeditor" id="editor5"><?php echo $listRow['contents']; ?></textarea>
...
<div> <!-- style="height: 300px; overflow: scroll;" -->
  <table width="100%">
    ...
    <?php
		...
		$sql = "SELECT * FROM groups WHERE parentId = '". $_GET['id'] . "' ORDER BY weight DESC, onDate DESC, id DESC";
